input <-
function(origin,destination,avoid=NULL,travel_mode=NULL){
    ...
    {mode<-NULL} else {mode<-"&mode="}
    if(is.null(avoid))
      {av<-NULL} else {av<-"&avoid="}
   ...
   zoom<-zoom.options[sum(difference<breaks)]
    ...
    al1MAP<-ggmap(al1)+geom_point(data=sol, aes(x=long, y=lat), color="red")+xlab("Longitude")+ylab("Latitude")
    return(al1MAP)
}
